...
class StatisticsService {
  ...
  Future<double> calculateUserBalance(String userUnified) async {
    final user = await _userRepository.get(userUnified);
  ...
  Future<double> calculateDailySales() async {
    ...
    final invoices = await _fatoraRepository.getBetweenDates(start, end);
  ...
  Future<double> calculateCashFlow() async {
    ...
    final payments = await _paymentRepository.getBetweenDates(start, end);
    final paymentsTotal = payments.fold(0.0, (s, p) => s + p.amount);

    final purchases = await _fatoraRepository.getBetweenDates(start, end);
  ...
  Future<List<String>> topProducts({int limit = 5}) async {
    final items = await _fatoraProductRepository.getAll();
